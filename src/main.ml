...
let speclist = [
  ...
  ("-trim", Arg.Int(fun n -> opt := { !opt with bufsize = Some(n) }), " [N] Let messages which have older versions than $latest - N$ removed from input buffers (it may cause glitches)")
]
